...
## 2. プログラミング言語では、使える命令が決まっている

さきほどのプログラムを、プログラミング言語で書くと・・・
```
run(2.0)
turn(90)
run(2.0)
...
ロボットを1m先にあるチェックポイント(goal1)を超えてから、元の位置(goal2)に戻らせよう。
...
- 命令は直後にwaitを置かない場合、どんどん下に実行されていってしまう。
    - 「一定時間だけ〇〇する」を最後まで実行するには、wait命令を直後に置く必要がある
...
ロボットを直進させながら標識(標識名はsign1)が車にぶつかりそう（左右0.2[m]以内）に見えた瞬間に停止しよう。
...
- 「標識が左右0.2[m]以内でなければ少しだけ前進する」プログラムのヒント
    1. 「標識が左右0.2[m]以内でない」という条件がどのように書けるか考えよう
    2. 条件を言い換えると「標識の左右位置が0.2[m]より大きい」もしくは「標識の左右位置が-0.2[m]より小さい」といえる。これをプログラムで書くとどうなる？
    3. 標識の左右位置はpos.yと書けるので、条件は「if pos.y > 0.2 or pos.y < -0.2:」と書ける
    4. if文の下には、「少しだけ前進する」プログラムを書こう。「少し」は例えば0.1[m]でOK。move命令のvとtを適切に設定しよう。
    5. move命令の速度(v)を0.2にした場合、tを「進む時間」=「進みたい距離(0.1)」/「速度（0.2）」となるように設定しよう。
    6. move命令の下には忘れずにwait()命令を書こう。
...
- ポイント
    ...
    - wait()を使わないことで、すぐに次の繰り返しが始まる
        - コンピュータの処理能力によるが、例えば10分の1秒～100分の1秒の速さで認識に応じた車のコントロールもできる

# チュートリアル4

ロボットを直進させながら標識(標識名はsign1)が車にぶつかりそう（左右0.2[m]以内）に見えた瞬間に停止しよう。※問題はチュートリアル3と同じ
...
- 「標識が左右0.2[m]以内なら停止し、そうでないなら一定速度で前進する」プログラムのヒント
    1. 「標識が左右0.2[m]以内」という条件がどのように書けるか考えよう
    2. 条件を言い換えると「標識の左右位置が0.2[m]以下」かつ「標識の左右位置が-0.2[m]以上」といえる。これをプログラムで書くとどうなる？
    3. 標識の左右位置はpos.yと書けるので、条件は「if pos.y <= 0.2 and pos.y >= -0.2:」と書ける
    4. if文の下には、「停止する」プログラムを書こう。move命令でv=0とすれば停止するぞ
    5. else文の下には、「前進する」プログラムを書こう。